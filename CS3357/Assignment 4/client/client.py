...
def main():
    ...
    #Connect to the load balancer to
    ...
    request = get_line_from_socket(client_socket)
    address = get_line_from_socket(client_socket)
    print('Received request:  ' + request)
    print(address)
    getRequest = get_line_from_socket(client_socket)
    getRequest2 = get_line_from_socket(client_socket)
    print(getRequest)
    print(getRequest2)
    response_list = address.split(":")
    host2 = response_list[1].lstrip()
    print("Priority host:" + host2)
    port2 = int(response_list[2])
    print("Priority port:" + str(port2))
    ...
    print('Connection to server established. Sending message...\n')
    message = prepare_get_message(host2, port2, file_name)
    client_socket.send(message.encode())
